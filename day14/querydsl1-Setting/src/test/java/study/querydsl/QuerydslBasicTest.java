...
//                .fetchResults();//count 쿼리로 변경
    ...
    public void join_on_filtering() throws Exception {
        ...
        // select가 여러가지 타입이면 튜플로 나온다.
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(member.team, team)
                .on(team.name.eq("teamA")) // left조인에서 의미가 있다.
//                .where(team.name,"teamA")
    ...
    public void join_on_no_relation() throws Exception {
        ...
        List<Tuple> result = queryFactory
                ...
                .from(member)
                .leftJoin(team)
                .on(member.username.eq(team.name)) // 조인하는 대상을 줄이는 역할을 한다.
                .fetch();
    ...
    public void fetchJoinNo() throws Exception {
        ...
        Member findMember = queryFactory
                ...
                .fetchOne();
        boolean loaded =
                emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam()); // findMember.getTeam()
    ...
    public void concat() throws Exception {
        List<String> result = queryFactory.select(member.username.concat("_")
                        .concat(member.age.stringValue())) // stringValue가 Enum처리할 떄 좋다.
                .from(member)
                .where(member.username.eq("member1"))
                .fetch();
    ...
    // 동적쿼리 boolean builder 사용
    // QueryDsl 문서에는 이 방식이 명시되어 있다.
    ...
    public void bulkupdate() throws Exception {
        ...
        for (Member member1 : result) {
            System.err.println("member = " + member1);
        }
    }
...
//        long count = queryFactory.update(member).set(member.age, member.age.multiply(2)).execute();
        System.err.println(count + "행이 변경되었습니다.");
    }
